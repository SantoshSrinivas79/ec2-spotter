...
request_id=$(aws ec2 describe-spot-instance-requests --output=json | jq -r '.SpotInstanceRequests[] | select( .State == "active" or .State == "open") | .SpotInstanceRequestId')

echo Waiting for spot request to be fulfilled... ID: $request_id
